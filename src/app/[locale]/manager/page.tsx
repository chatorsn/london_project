    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }} style={{ background: "#0a0a12", minHeight: "100vh", fontFamily: "var(--font-jost), sans-serif" }}>
      ...
      <main style={{ padding: "32px" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          <motion.div initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.4, delay: 0.1 }} style={{ display: "flex", justifyContent: "space-between", marginBottom: 32, gap: 16, flexWrap: "wrap" }}>
            <div style={{ display: "flex", gap: 12 }}>
              {statuses.map((status, idx) => (
                <motion.button key={status} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: idx * 0.05 }} onClick={() => setStatusFilter(status)} style={{ background: statusFilter === status ? "rgba(255,255,255,0.1)" : "rgba(255,255,255,0.03)", border: "0.5px solid rgba(200,215,235,0.2)", padding: "8px 20px", color: statusFilter === status ? "rgba(225,232,245,0.9)" : "rgba(170,185,210,0.6)", fontSize: "10px", letterSpacing: "0.1em", textTransform: "uppercase", cursor: "pointer" }} whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>{t(`status${status.charAt(0).toUpperCase() + status.slice(1)}`)}</motion.button>
              ))}
            </div>
            <motion.input initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ duration: 0.4, delay: 0.2 }} type="text" placeholder={t("search")} value={search} onChange={(e) => setSearch(e.target.value)} style={{ background: "rgba(255,255,255,0.03)", border: "0.5px solid rgba(200,215,235,0.2)", padding: "10px 16px", color: "rgba(225,232,245,0.85)", fontSize: "12px", minWidth: 250, outline: "none" }} />
          </motion.div>
          ...
          <AnimatePresence mode="wait">
            {filteredBookings.length === 0 ? (
              ...
              <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead>
                    <tr style={{ borderBottom: "0.5px solid rgba(200,215,235,0.15)", textAlign: "left" }}>
                      {["tableId", "tableCustomer", "tableService", "tableRoute", "tableDateTime", "tablePassengers", "tableStatus"].map((col, idx) => (<th key={col} style={{ padding: "16px 12px", fontSize: "10px", color: "rgba(170,185,210,0.5)", fontWeight: 300 }}>{t(col)}</th>))}
                    </tr>
                  </thead>
                  <tbody>
                    {filteredBookings.map((booking, idx) => (
                      <motion.tr key={booking.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} style={{ borderBottom: "0.5px solid rgba(200,215,235,0.08)" }} whileHover={{ background: "rgba(255,255,255,0.03)" }}>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} style={{ marginTop: 24, padding: "16px 0", borderTop: "0.5px solid rgba(200,215,235,0.08)", display: "flex", justifyContent: "space-between" }}>
