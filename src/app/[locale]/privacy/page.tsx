...
const bookingSchema = z.object({
  ...
  phone: z.string().min(5, "validation.required"),
  email: z.string().email("validation.email"),
  serviceType: z.string().min(1, "validation.required"),
  pickup: z.string().min(1, "validation.required"),
  ...
  passengers: z.number().min(1, "validation.passengers"),
  meetAndGreet: z.boolean().optional(),
  notes: z.string().optional(),
  consent: z.boolean().refine(val => val === true, "validation.consent"),
});
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg shadow-lg p-8 md:p-12 space-y-6">
          <div className="grid md:grid-cols-2 gap-6">
            <div>
              ...
              <input {...register("fullName")} className="w-full border rounded-lg p-3" />
              {errors.fullName && <p className="text-red-500 text-sm mt-1">{t(errors.fullName.message as any)}</p>}
            </div>
            ...
            <div>
              ...
              {errors.phone && <p className="text-red-500 text-sm mt-1">{t(errors.phone.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.email")} *</label>
              <input type="email" {...register("email")} className="w-full border rounded-lg p-3" />
            ...
            <div>
              ...
              <select {...register("serviceType")} className="w-full border rounded-lg p-3">
              ...
              {errors.serviceType && <p className="text-red-500 text-sm mt-1">{t(errors.serviceType.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.passengers")} *</label>
              <input type="number" {...register("passengers", { valueAsNumber: true })} className="w-full border rounded-lg p-3" />
              {errors.passengers && <p className="text-red-500 text-sm mt-1">{t(errors.passengers.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.pickup")} *</label>
              <input {...register("pickup")} className="w-full border rounded-lg p-3" />
              {errors.pickup && <p className="text-red-500 text-sm mt-1">{t(errors.pickup.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.destination")} *</label>
              <input {...register("destination")} className="w-full border rounded-lg p-3" />
              {errors.destination && <p className="text-red-500 text-sm mt-1">{t(errors.destination.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.date")} *</label>
              <input type="date" {...register("date")} className="w-full border rounded-lg p-3" />
              {errors.date && <p className="text-red-500 text-sm mt-1">{t(errors.date.message as any)}</p>}
            </div>

            <div>
              <label className="block text-sm font-medium mb-2">{t("form.time")} *</label>
              <input type="time" {...register("time")} className="w-full border rounded-lg p-3" />
              {errors.time && <p className="text-red-500 text-sm mt-1">{t(errors.time.message as any)}</p>}
            </div>
          </div>
          ...
          <div>
            <label className="flex items-start gap-2">
              <input type="checkbox" {...register("consent")} className="w-4 h-4 mt-1" />
              <span className="text-sm text-gray-600">{t("form.consent")}</span>
            </label>
            {errors.consent && <p className="text-red-500 text-sm mt-1">{t(errors.consent.message as any)}</p>}
          </div>
